...
const FeaturedCard: FC<FeaturedCardProps> = ({ index, setCurrentBlogHandler, title, description, image, imageAlt, imageStyle, descriptionStyle }) => {
  return <div
    onClick={() => setCurrentBlogHandler(index)}
    className={
      cn('px-2 sm:px-4 rounded-lg overflow-hidden cursor-pointer')
    }>
    <div className="flex item-center gap-1 sm:gap-2">
      <GatsbyImage
        ...
        className={cn('w-20 md:w-36 md:h-24 rounded-lg', imageStyle)}
      />
