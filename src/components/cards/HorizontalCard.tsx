  ...
  return <Link to={`/blogs/${slug}`} className={
    ...
    <div className="flex item-center gap-2 sm:gap-4">
      ...
      <div className="flex-1 flex flex-col justify-between ml-2 sm:ml-4">
        <p className='line-clamp-2 text-black text:lg sm:text-2xl my-1 sm:my-2 font-semibold'>
          {title}
        </p>
        <p className={cn('line-clamp-1 sm:line-clamp-5 text-black/70 sm:text-lg', descriptionStyle)}>
          {description}
        </p>
        <div className="flex gap-1 justify-end sm:justify-start items-end">
          <p className='text-black text-sm sm:text-lg sm:font-medium'>المزيد</p>
          <ArrowLeft className='text-black text-sm sm:text-lg' size={20} />
        </div>
      </div>
    </div>
  </Link>
